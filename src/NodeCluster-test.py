...
class NodeCluster(object):
    ...
    def insertToMaxCuboid(self, queue):
        ...
        while True:
            ...
            if jobsize > plane:
                for index in xrange(startindex, startindex+jobsize/plane):
                    for i in xrange(0,rowlength):
                        for j in xrange(0,collength):
                            self.matrix[index,i,j] = currentjob.returnId()
                            cood.append([index,i,j])
                jobsize = jobsize % plane
                startindex = startindex + jobsize/plane
