...
class TestBackfilling(unittest.TestCase):
    def setUp(self):
        self.timer = Timer()
        self.cluster = Cluster(Point3D(1, 1, 2), plotting=False, timer=self.timer, backfill_depth=1, buffer_size=5)
    ...
    def test_backfilling(self):
        ...
        job_list = [
            ...
            Job(19, 1, 1, 1, self.timer, self.cluster),
        ]
    ...
    def test_backfilling_1(self):
        self.cluster = Cluster(Point3D(2, 1, 3), plotting=False, timer=self.timer, backfill_depth=1, buffer_size=4)
